    ...
    def __getitem__(self, idx):
        ...
        """
         try:
            ...
            return None
        """
        if roll[i].shape[0]>=20016:
            return None
        try:
            for i in range(5):
                #print(roll[i].shape[0])
                temproll.append(np.vstack((roll[i], np.zeros((20016 - roll[i].shape[0], 128)))))
        except:
            return None
        # temproll=np.vstack((data[i].tracks[2].pianoroll,temp))
        # temproll=np.vstack((temproll,np.zeros((20016-temproll.shape[0],128))))
        ...
        for i in range(5):
            temproll[i] = downsampling(20016, temproll[i])
...
if __name__=='__main__':
 ...
 """
    ...
    for j in range(int(21425/7)):
        ...
        pickle.dump(rolls, open('D:/1projects/MusicGan/styleMuseGAN/down_sampling_data/d_20016_{}.pke'.format(j), 'wb'))
        rolls=[]
        p.close()
        p.join()
    #print('等待所有子进程完成。')
